...
0.41.0 reduce trabajo repetido y cierra varios huecos de lifecycle sin ampliar Jobs hacia gameplay.
...
- F5/Actualizar en Multiplayer conserva **servidor seleccionado y posición de scroll**. `servers.dat` sólo se guarda si realmente hubo alta, baja, renombre, deduplicación o movimiento del servidor oficial.
...
- El diagnóstico oculto añade cierres efectivos de sesión, FX puntuales, purgas y métricas de cambios/guardados de config.
- Nuevo `tools/verificar_runtime_041.py` fija estos contratos en CI.
...
GitHub Actions ejecuta política de versión, fondos, verificador general, UI/música, continuidad Multiplayer/docs, optimización, créditos/reload, identidad musical/hard-stop y el nuevo contrato runtime 0.41 antes del build Forge real con Java 17. `dev-latest` sólo se publica desde `main` verde.
