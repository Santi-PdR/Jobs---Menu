...
def dimensiones_png(datos:bytes)->tuple[int,int]:
    if len(datos)<24 or datos[:8]!=b'\x89PNG\r\n\x1a\n' or datos[12:16]!=b'IHDR': raise ValueError('firma/IHDR PNG invalida')
...
def materializar(indice:int)->None:
    if indice==14:return
    if indice==10:nombre='nivel10.part00'
    elif indice<=13:nombre=f'nivel{indice}.fixed.b64'
    else:nombre=f'nivel{indice}.b64'
    origen=PAYLOAD/nombre
    if not origen.is_file():raise FileNotFoundError(f'falta {origen.relative_to(RAIZ)}')
...
def main()->int:
    try:
        for i in range(10,18):materializar(i)
        print('Fondos de imagen:')
        for i in range(10,18):validar(i)
    except Exception as exc:
        print(f'ERROR fondos: {exc}',file=sys.stderr);return 1
    return 0
