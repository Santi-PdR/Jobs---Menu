...
public final class EscenaNivel {
    ...
    public static void dibujar(GuiGraphics grafico, int ancho, int alto) {
        ...
        float luz = brilloFluorescente(tiempo, destellos)
                * (1.0F - 0.55F * penumbra) * RotacionNiveles.luzDisponible();
        if (movimiento) luz *= Presencia.sombra();
        luz = Trazo.limitar(luz, 0.0F, 1.0F);

        Marco marco = new Marco(ancho, alto, ancho * nivel.fugaX, alto * nivel.fugaY,
        ...
        DetallesRecinto.dibujar(grafico, nivel, marco, luz, tiempo);
    ...
    private static void motas(GuiGraphics g, int ancho, int alto, float tiempo, float luz) {
        for (int i = 0; i < MOTAS; i++) {
            float baseX = Trazo.pseudo(i * 7), baseY = Trazo.pseudo(i * 7 + 1);
            float velocidad = 0.10F + Trazo.pseudo(i * 7 + 2) * 0.30F;
            float deriva = (float) Math.sin(tiempo * (0.25F + Trazo.pseudo(i * 7 + 3) * 0.4F) + i) * 0.012F;
            int px = (int) (((baseX + deriva + 1.0F) % 1.0F) * ancho);
            int py = (int) (((baseY + tiempo * velocidad * 0.045F) % 1.0F) * alto);
            int tam = Trazo.pseudo(i * 7 + 4) < 0.80F ? 1 : 2;
            float a = (0.10F + Trazo.pseudo(i * 7 + 5) * 0.22F) * luz;
            g.fill(px, py, px + tam, py + tam, Paleta.conAlfa(Paleta.FLUOR, a));
        }
    }
    ...
    private static void vineta(GuiGraphics g, int ancho, int alto, float penumbra) {
        ...
        for (int x = 0; x < franja; x += paso) {
            ...
            g.fill(x, 0, x + paso, alto, color);
            g.fill(ancho - x - paso, 0, ancho - x, alto, color);
        }
        ...
        for (int y = 0; y < franjaV; y += paso) {
            ...
            g.fill(0, y, ancho, y + paso, color);
            g.fill(0, alto - y - paso, ancho, alto - y, color);
        }
    }

    public static float brilloFluorescente(float tiempo, boolean destellos) {
        if (!destellos) return 0.90F;
        float v = 0.90F + 0.035F * (float) Math.sin(tiempo * 1.7F)
                + 0.020F * (float) Math.sin(tiempo * 5.9F + 1.3F);
        if (Math.floorMod((long) (tiempo * 3.0F), 97L) == 0L) v *= 0.62F;
