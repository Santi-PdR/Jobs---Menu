...
public final class EventosAmbientales {
    ...
    public static void dibujar(GuiGraphics grafico, int ancho, int alto,
        ...
        dibujar(grafico, ancho, alto, nivel, luz, System.currentTimeMillis());
    }

    /** Variante que comparte el instante capturado por EscenaNivel. */
    public static void dibujar(GuiGraphics grafico, int ancho, int alto,
                               Nivel nivel, float luz, long ahora) {
        ...
        if (numero <= 2) {
            ...
            siluetaLejana(grafico, ancho, alto, pulso, semilla, progreso);
            cascoteTrono(grafico, ancho, alto, nivel, luz, pulso, semilla, progreso);
        } else {
            polvoEnHaz(grafico, ancho, alto, nivel, luz, progreso, pulso, semilla);
        }
    }
    ...
    /**
     * Un cascote cae fuera del eje del trono durante el evento del Nivel 9.
     *
     * No busca asustar ni competir con el asiento: nace en un lateral, cae una
     * distancia corta y desaparece antes de tocar la tarima. El progreso es el
     * mismo que la ventana del evento, asi que no puede saltar por usar otro
     * reloj.
     */
    private static void cascoteTrono(GuiGraphics g, int ancho, int alto,
                                     Nivel nivel, float luz, float pulso,
                                     int semilla, float progreso) {
        boolean derecha = (semilla & 8) == 0;
        float posicion = 0.18F + 0.64F * pseudo(semilla + 53);
        if (derecha) {
            posicion = 0.62F + 0.25F * pseudo(semilla + 59);
        } else {
            posicion = 0.13F + 0.25F * pseudo(semilla + 59);
        }
        int x = (int) (ancho * posicion);
        int y = (int) (alto * (0.20F + progreso * 0.34F));
        int tam = Math.max(2, ancho / 190);
        int piedra = Paleta.mezclar(nivel.junta, nivel.paredBaja, 0.45F);
        g.fill(x, y, x + tam, y + tam, Paleta.conAlfa(Paleta.iluminar(piedra, luz), 0.72F * pulso));
        g.fill(x - 1, y + tam, x + tam + 2, y + tam + 1,
                Paleta.conAlfa(Paleta.VANO, 0.28F * pulso));
    }
