...
`retornoDesdeJuego` reconducía Title, Multiplayer o Realms siempre a `PantallaNivel`. Eso impedía caer en vanilla, pero también descartaba el contexto de servidor.

### Corrección

- destino `JoinMultiplayerScreen` tras `LoggingOut` → `PantallaMultijugadorJobs(new PantallaNivel())`;
- destino Title/Realms → `PantallaNivel`;
...
3. salir manualmente del servidor → lista Jobs;
4. kick/pérdida de conexión → puede mostrarse primero el mensaje de desconexión y, al continuar a Multiplayer, se reconduce a lista Jobs;
5. ESC desde esa lista → main Jobs en una acción.
...
4. Salir manualmente del servidor; debe abrir Multijugador Jobs.
